...
public:
	 ...
	 * @param col the col number (zero indexed)
	 */
	T operator()(const int32_t row, const int32_t col) const;
...
template <class T>
T& Matrix<T>::operator()(const int32_t row, const int32_t col) {
	return _data[row * _cols + col];
}

template <class T>
T Matrix<T>::operator()(const int32_t row, const int32_t col) const {
	return _data[row * _cols + col];
}
